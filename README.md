# Olá! Eu sou o Marcello 👋

### Professor de Matemática e Física | Estudante de Engenharia da Computação

Sou professor de **Matemática e Física** e estudante de **Engenharia da Computação**.

Minha trajetória une duas áreas que sempre fizeram parte da minha vida: **Educação e Tecnologia**.

Atualmente, estou aprofundando meus conhecimentos em programação, algoritmos, estruturas de dados, engenharia de software e desenvolvimento de aplicações.

---

## 🧑‍🏫 Sobre mim

* 🎓 Professor de Matemática e Física
* 💻 Estudante de Engenharia da Computação — UNIVESP
* 🧮 Apaixonado por Matemática e resolução de problemas
* ⚛️ Interesse por Física e suas aplicações
* 🐍 Estudando Python
* 💻 Estudando C++
* 🧠 Estudando Algoritmos e Estruturas de Dados
* 🏗️ Estudando Engenharia de Software
* 🤖 Interesse em Inteligência Artificial e Machine Learning
* 📚 Tecnologia aplicada à educação
* ⚽ Apaixonado por futebol

---

## 💻 Tecnologias

### Linguagens

* Python
* C++

### Computação

* Lógica de Programação
* Algoritmos
* Estruturas de Dados
* Programação Orientada a Objetos
* Engenharia de Software
* Banco de Dados
* Sistemas Embarcados
* Internet das Coisas (IoT)
* Inteligência Artificial
* Machine Learning

---

## 🎓 Formação

### Engenharia da Computação

**UNIVESP — Universidade Virtual do Estado de São Paulo**

Atualmente desenvolvendo conhecimentos em programação, algoritmos, estruturas de dados, engenharia de software, sistemas, inteligência artificial e outras áreas da computação.

### Matemática e Física

Professor com experiência em ensino de Matemática e Física, áreas que contribuem diretamente para minha formação em computação através do raciocínio lógico, pensamento analítico e resolução de problemas.
Formado em Física pela UNESP e Matemática pela UNISAL

---

## 🚀 Atualmente estudando

```text
Python
C++
Algoritmos
Estruturas de Dados
Engenharia de Software
Inteligência Artificial
Machine Learning
```

---

## 🎯 Objetivos

* Aprimorar minhas habilidades de programação
* Desenvolver projetos cada vez mais completos
* Construir uma base sólida em computação
* Aprender boas práticas de Engenharia de Software
* Explorar Inteligência Artificial e Machine Learning
* Criar soluções tecnológicas aplicadas à educação
* Construir um portfólio profissional

---

## 📂 Projetos

Aqui estarão reunidos meus projetos acadêmicos, experimentos de programação e projetos pessoais desenvolvidos ao longo da minha jornada na Engenharia da Computação.

---

## 📚 Áreas de interesse

* 💻 Desenvolvimento de Software
* 🧠 Algoritmos e Estruturas de Dados
* 🤖 Inteligência Artificial
* 📊 Ciência de Dados
* 🧮 Matemática Computacional
* ⚛️ Computação aplicada à Física
* 🎓 Tecnologia aplicada à Educação
* 🌐 Internet das Coisas
* ⚙️ Sistemas Embarcados

---

## 📫 Vamos nos conectar?

Se você também gosta de **programação, Matemática, Física, tecnologia ou educação**, será um prazer trocar conhecimento.

---

### "Transformar conhecimento em solução."

**Marcello**
